...
const TabNavigation = () => {
    ...
    const tabs = [
        { name: "Dashboard", link: "/dashboard", roles: ["BU Level", "SDG Owner", "Convener", "Stakeholder", "BU Head", "Convener L2"] },
        { name: "Add new note", link: "/AddProduct", roles: ["BU Level", "SDG Owner"] },
    ...
    return (
        <Nav className="justify-content-start mb-3 bg-white rounded p-1">
            {tabs
                .filter(tab => tab.roles.includes(user?.roles))
                .map((tab) => (
